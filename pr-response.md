...
- **Stress-test results:**
  - _Comment 4 (visibility):_ The devil's-advocate pass raised three counterarguments: (1) **cold-start starvation** — for an early-stage community app, private-by-default starves the discovery graph exactly when the network is most fragile, and my "opt-in UX will recover it" mitigation *doesn't exist yet*; (2) that watchlist exposure may be less sensitive than I claimed, so I might be over-weighting a minor privacy harm; and (3) a **consistency gap** — `CollectionEntry` has no `public` field and there's no endpoint to view another user's public watchlist, so the public/private distinction is partly moot today. What I changed: I strengthened the *Tradeoff acknowledged* paragraph to concede the cold-start cost explicitly and to explain *why* it's the recoverable side of the asymmetry (opt-in prompts can be added later; over-exposure can't be undone). I held the private-by-default position because that asymmetry argument survives the pushback.
